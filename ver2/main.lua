...
function love.load()
    ...
    -- Single file audio support
    -- music = love.audio.newSource("space_music.wav", "stream")
    ...
    -- Load each music file into the tracks table
    for i, file in ipairs(musicFiles) do
        ...
        if extension == "mp3" or extension == "ogg" or extension == "wav" then
            local track = love.audio.newSource("music/" .. file, "stream")
            table.insert(musicTracks, track)
        end
    end
...
function createColoredStars()
    ...
    local coloredStarCount = love.math.random(40, 60) -- Rare compared to white stars
...
function createAsteroids()
    local asteroidCount = love.math.random(10, 20)
    ...
    for i = 1, asteroidCount do
        ...
        local vertexCount = love.math.random(6, 12)
...
function createComets()
    ...
    local cometCount = love.math.random(2, 5)
    ...
    for i = 1, cometCount do
        ...
        local size = love.math.random(4, 6)
        ...
        local comet = {
            ...
            trailLength = love.math.random(20, 40), -- Length of trail in segments
            trailThickness = size * 0.6 -- Thickness of trail
        }
...
function createWormhole()
    ...
    local newWormhole = {
        ...
        outerRadius = love.math.random(60, 70), -- Size of the wormhole
        innerRadius = love.math.random(50, 60),
        rotation = 0,
...
-- Function to shuffle our tracks
...
function love.update(dt)
    ...
    if currentTrack and not currentTrack:isPlaying() then
        ...
        -- If we've reached the end of our playlist, reshuffle and start over
